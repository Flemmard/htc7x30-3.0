...
static int sonywvga_panel_power(int on)
{
	...
	if (gpio_switch) {
        ...
        printk(KERN_ERR "%s: power good!\n", __func__);

	return 0;

power_fail:
        printk(KERN_ERR "%s: power fail!\n", __func__);
	return ret;
}
...
static int sonywvga_panel_unblank(struct msm_lcdc_panel_ops *panel_data)
{
  LCMDBG("%s %d\n", __func__, g_down);
        ...
        //        if (g_down)
        //          sonywvga_panel_init(panel_data);
        wake_lock(&panel_idle_lock);
        mutex_lock(&panel_lock);
...
static struct msm_lcdc_timing sonywvga_lcdc_timing = {
  //	.clk_rate		= 22576000,	//In Bravoc, Radio will adjust clk rate from 24.57Mhz to 23.59Mhz to pass EMC testing
