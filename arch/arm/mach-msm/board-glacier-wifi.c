...
int glacier_wifi_get_mac_addr(unsigned char *buf);
...
static struct wifi_platform_data glacier_wifi_control = {
	...
	.get_mac_addr	= glacier_wifi_get_mac_addr,
	.dot11n_enable  = 1,
};
...
int glacier_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"glacier_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
