...
extern int glacier_wifi_bt_sleep_clk_ctl(int on, int id);
...
static void glacier_config_bt_init(void)
{
	...
	/* BT_SHUTDOWN_N */
	gpio_set_value(GLACIER_GPIO_BT_SHUTDOWN_N,
				1);
	mdelay(2);
	/* BT_RESET_N */
	gpio_set_value(GLACIER_GPIO_BT_RESET_N,
				1);
	mdelay(15);
	...
	/* BT_RTS */
	gpio_set_value(GLACIER_GPIO_BT_UART1_RTS,
				1);
	/* BT_CTS */
	...
	/* BT_TX */
	gpio_set_value(GLACIER_GPIO_BT_UART1_TX,
				0);

	/* BT_CHIP_WAKE */
	gpio_set_value(GLACIER_GPIO_BT_CHIP_WAKE,
				0);
...
int __init glacier_rfkill_init(void)
{
